...
// 1) marcar os obstaculos
 for (int x = minKnownX_; x <= maxKnownX_; x++){
        for (int y = minKnownY_; y <= maxKnownY_; y++){
            ...
            }else if (occupancyTypeGrid_[i] >= 70){
                occupancyTypeGrid_[i] = OCC_OCCUPIED;
            }else{
...
// 2) marcar as perto dos obstaculos
    for (int x = minKnownX_; x <= maxKnownX_; x++){
        for (int y = minKnownY_; y <= maxKnownY_; y++){
            int i = x + y*numCellsX_;

            if (occupancyTypeGrid_[i] == OCC_OCCUPIED){
                for (int dx = -dangerZoneWidth; dx <= dangerZoneWidth; dx++){
                    for (int dy = -dangerZoneWidth; dy <= dangerZoneWidth; dy++){
                        int nx = x + dx;
                        int ny = y + dy;
                        if (nx >= 0 && nx < numCellsX_ && ny >= 0 && ny < numCellsY_){ // ve se ta dentro do mapa
                            int ni = nx + ny * numCellsX_;
                            if (occupancyTypeGrid_[ni] == OCC_FREE){
...
// 3) marcar as fronteiras
    for (int x = minKnownX_; x <= maxKnownX_; x++){
        for (int y = minKnownY_; y <= maxKnownY_; y++){
            int i = x + y * numCellsX_;

            if (occupancyTypeGrid_[i] == OCC_FREE){
                bool hasUnexploredNeighbor = false;
                ...
                for (int dx = -1; dx <= 1; dx++){
                    for (int dy = -1; dy <= 1; dy++){
                        ...
                        if (nx >= 0 && nx < numCellsX_ && ny >= 0 && ny < numCellsY_){
                            int ni = nx + ny * numCellsX_;
                            if (occupancyTypeGrid_[ni] == OCC_UNEXPLORED){
                                hasUnexploredNeighbor = true;
                                break;
                            }
                        }
                    }
                    if (hasUnexploredNeighbor) break;
                }
...
// 4) marca o resto como livre (precisa? já fiz na parte 1)
...
int Perception::computeShortestPathToFrontier(int robotCellIndex)
{
    ...
    while (!pq.empty()){
        // Retira célula com menor f da fila
        auto current = pq.top();
        pq.pop();

        int currentIndex = current.second;
        int cx = currentIndex % numCellsX_;
        int cy = currentIndex / numCellsX_;

        if (planningTypeGrid_[currentIndex] == PLAN_GOALS){
            goal = currentIndex;
            break; // pode parar a busca pq achou um objetivo
        }
        ...
        for (int i = 0; i < 8; i++){
            ...
            if (nx < minKnownX_ || nx > maxKnownX_ || ny < minKnownY_ || ny > maxKnownY_) continue; // se ta fora do mapa sai

            int neighborIndex = nx + ny * numCellsX_;

            if (planningTypeGrid_[neighborIndex] == PLAN_INVALID) continue; 

            double new_g = gValueGrid_[currentIndex] + cost[i];

            if (new_g < gValueGrid_[neighborIndex]){
                gValueGrid_[neighborIndex] = new_g;
                fValueGrid_[neighborIndex] = new_g + hValueGrid_[neighborIndex];
                parentGrid_[neighborIndex] = currentIndex;

                pq.push({fValueGrid_[neighborIndex], neighborIndex});
            }
        }
    }
