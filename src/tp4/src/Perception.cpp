...
void Perception::MCL_sampling(const Action &u)
{
    ...
    std::normal_distribution<double> sampler_rot1(u.rot1, uncertainty);
    std::normal_distribution<double> sampler_trans(u.trans, uncertainty);
    std::normal_distribution<double> sampler_rot2(u.rot2, uncertainty);

    for (int i = 0; i < numParticles_; ++i){
        double rot1 = sampler_rot1(*generator_);
        double trans = sampler_trans(*generator_);
        double rot2 = sampler_rot2(*generator_);
...
void Perception::MCL_weighting(const std::vector<float> &z)
{
   /// TODO: faça a pesagem de todas as particulas
    ...
    for (int i = 0; i < numParticles_; i++){
        ...
        if (ix >= static_cast<unsigned int>(numCellsX_) || iy >= static_cast<unsigned int>(numCellsY_)){ // fora do mapa
            particles_[i].w = 0.0;
            continue;
        }
        if (gridMap_.data[indice] != 0){ // nao é espaço livre
            particles_[i].w = 0.0;
            continue;
        }
        ...
        for(size_t k = 0; k < z.size(); k +=10){
            double real_zk = z[k];
            double expected_zk = computeExpectedMeasurement(k, particles_[i].p);
            double diff = real_zk - expected_zk;

            double prob = (1.0 / sqrt(2.0*M_PI*var)) * exp(-(diff*diff) / (2.0*var));
...
*/
    ...
    for(int j = 0; j < numParticles_; j++){
        double u = r + (static_cast<double>(j) / numParticles_);
        while(u > c){
            i++;
            if (i >= numParticles_) i = numParticles_ - 1;
            c += particles_[i].w;
        }
        nextGeneration.push_back(particles_[i]);
        nextGeneration.back().w = 1.0 / numParticles_;
    }
